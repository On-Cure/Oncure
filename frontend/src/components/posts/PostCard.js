...
import TipButton from "../tokenomics/TipButton";
      ...
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <VerificationBadge 
              role={post.user?.role} 
              verificationStatus={post.user?.verification_status} 
              size="sm" 
            />
          </div>
    ...
    <div className="flex flex-wrap gap-1 sm:gap-4 text-text-secondary px-1 pt-2 border-t border-border/20">
      ...
      <div className="ml-auto">
        <TipButton
          ...
          recipientName={`${post.user?.first_name || 'User'} ${post.user?.last_name || ''}`}
          onTipSent={(transaction) => {
            console.log('Tip sent:', transaction);
